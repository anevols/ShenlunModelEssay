...
export function parseHtmlFile(htmlText, fileName) {
  ...
  const headStyles = Array.from(doc.querySelectorAll('head style, body style'))
    .map((n) => n.outerHTML)
    .join('\n')
    ...
    if (firstH1) {
      ...
      if (
        ...
        (nextAfterH1.classList.contains('article-meta') ||
          /(text-align\s*:\s*center|color\s*:\s*#(888|999|9ca|6b72|99a))/i.test(
            nextAfterH1.getAttribute('style') || ''
          ) ||
          (/^(p|div)$/i.test(nextAfterH1.tagName) &&
            /居中|点击|提示|解析|标签/.test(nextAfterH1.textContent || '')))
      ) {
        nextAfterH1.remove()
      }
    }
