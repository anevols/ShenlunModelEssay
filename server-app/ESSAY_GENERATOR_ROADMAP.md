...
```
server-app/server/
├─ main.py                  ← 新增 /api/admin/generate 路由
├─ models.py                ← Article 模型（复用，不改）
├─ categories.py            ← 十大板块（复用，不改）
├─ auth.py                  ← JWT/管理员守卫（复用，不改）
├─ generator/               ← 新增：范文生成器
│  ├─ __init__.py
│  ├─ config.py             ← LLM 配置（模型、API key、温度）
│  ├─ topics.py             ← 主题/分论点/案例素材加载（基于 categories.py）
│  ├─ prompts.py            ← 系统 Prompt + 动态 Prompt 组装
│  ├─ llm_client.py         ← OpenAI SDK 封装（同步调用 + JSON 模式）
...
- [ ] 实现 `config.py`：LLM 配置（从环境变量读取 API key、模型名、温度）
- [ ] 实现 `topics.py`：基于 `categories.CATEGORIES` 加载主题，附分论点方向与案例素材（内置素材池，JSON 常量）
- [ ] 实现 `llm_client.py`：OpenAI SDK 封装（`chat.completions.create` + `response_format: json_object`）
...
- [ ] **最小闭环验证**：CLI 脚本 `python -m generator` 指定主题 → 调 LLM → 打印 HTML

**交付目标：** 跑通「指定主题 → LLM → HTML 字符串」，HTML 能被 `parseHtmlFile` 正确解析。
...
- [ ] 案例素材库管理：后台增加素材库 CRUD 页面（可选，存入 `Setting` 模型或新增 `Case` 表）
...
| API 鉴权 | 新路由 `POST /api/admin/generate` 用 `Depends(get_admin_user)` |
...
| 前端路由 | `/admin/generate` 作为 `AdminLayout` 子路由，复用后台侧边栏与权限守卫 |
